...
void process_command(adobe::application_t* app)
{
    ...
    {
    case ADOBE_ABOUT:
        gtk_dialog_run(GTK_DIALOG(gtk_message_dialog_new(nullptr,
...
{
    auto env_path = std::getenv("ADOBE_PLATFORM_BEGIN_RESOURCES_PATH");
    auto res_path = env_path ? env_path : ADOBE_PLATFORM_BEGIN_RESOURCES_PATH;
    app->set_resource_directory(res_path);
...
int main(int argc, char** argv)
{
    try
    {
        ...
        if (auto app = adobe::application_t::getInstance())
        {
            app->run();
        }
    } catch (...)
