...
namespace {
void process_change(GFileMonitor     *monitor,
               GFile            *file,
               GFile            *other_file,
...
void file_monitor_platform_data_t::connect()
{
    ...
    change_handle_m.monitor_m = g_file_monitor_file (g_file_new_for_path(path_m.native().c_str()), G_FILE_MONITOR_NONE, NULL, NULL);
    change_handle_m.signal_handler_id = g_signal_connect (change_handle_m.monitor_m, "changed", G_CALLBACK (process_change), this);
}
...
void file_monitor_platform_data_t::disconnect()
{
    ...
    if (change_handle_m.monitor_m)
    {
        ...
        {
            g_signal_handler_disconnect (change_handle_m.monitor_m, change_handle_m.signal_handler_id);
