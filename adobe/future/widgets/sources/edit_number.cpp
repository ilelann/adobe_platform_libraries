...
template <>
struct delete_ptr_trait<adobe_cursor_t>
{
    void operator()(adobe_cursor_t x) const
    { if (x) delete_cursor(x); }
};
...
{
    static auto_resource<adobe_cursor_t> scrubby_cursor;
    ...
    if (!inited)
    {
        ...
        scrubby_cursor.reset(make_cursor("cursor_scrub_slider.tga", 15, 15));
    }

    return scrubby_cursor.get();
}
